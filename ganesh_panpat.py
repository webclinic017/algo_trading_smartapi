    ...
    try:
      if st.session_state[symbol+'_Trade']=="-":
        ...
        for i in range(0,len(option_list)):
          ...
          if (opt_data['ST_7_3 Trade'].values[-1]=="Buy" or opt_data['ST_10_2 Trade'].values[-1]=="Buy" or opt_data['RSI_60 Trade'].values[-1]=="Buy"):
...
def get_todays_trade(orderbook):
  ...
  n_buy_df=buy_df[['updatetime','tradingsymbol','price','ordertag','Exit Time','Status', 'Sell', 'ltp', 'Profit','Target',
       'Stop Loss', 'Profit %', 'Sell Indicator']]
    ...
    if buy_df['Status'].iloc[i]=="Pending":
      try:
        ...
        if (opt_data['ST_7_3 Trade'].values[-1]=="Sell" or opt_data['ST_10_2 Trade'].values[-1]=="Sell"):
          exit_position(symboltoken,tradingsymbol,exch_seg,qty,ltp_price,ltp_price,ordertag='Indicator Exit:'+str(buy_df['ltp'].iloc[i])+" "+str(orderid),producttype='CARRYFORWARD')
    ...
    try:
      ...
      while now.second <50:
        ...
        if now.second <30:
          ...
          n_buy_df=todays_trade[['updatetime','tradingsymbol','price','ordertag','Exit Time','Status', 'Sell', 'ltp', 'Profit','Target',
                               'Stop Loss', 'Profit %', 'Sell Indicator']]
          todays_trade_datatable.dataframe(n_buy_df,hide_index=True)
          todays_trade_updated.text(f"Todays Trade : {datetime.datetime.now(tz=gettz('Asia/Kolkata')).time().replace(microsecond=0)} PNL: {pnl}")
        now=datetime.datetime.now(tz=gettz('Asia/Kolkata'))
