...
def get_historical_trade(dat):
  df=pd.DataFrame()
  for symbol in ['^NSEI','^NSEBANK']:
    for interval in ['5m','15m']:
      df_1=get_historical_data(symbol=symbol,interval=interval,token="-",exch_seg="-",candle_type='NORMAL')
      df_1['Buy Time']='-'
      for i in range(0,len(df_1)-1):
        df_1['Buy Time'].iloc[i]=df_1['Datetime'].iloc[i+1]
      df=df.append(df_1)
    df=df.sort_values(by = ['Date', 'Datetime','Time Frame'], ascending = [True, True,False], na_position = 'first')
  df=df[(df['Date']==dat)]
  df=df[(df['Trade End']!='-') & (df['Trade']!='-')]
  df['End Time']='-';df['Trade Exit']='Day End';df['Options']='-';df['token']='-';df['qty']='-'
  df['End Time']=datetime.datetime.now(tz=gettz('Asia/Kolkata')).replace(hour=15, minute=29, second=0, microsecond=0, tzinfo=None).time()
  for i in range(0,len(df)-1):
    symbol=df['Symbol'].iloc[i]
    trade=df['Trade'].iloc[i]
    for j in range(i,len(df)):
      if df['Symbol'].iloc[j]==symbol and df['Trade'].iloc[j]!=trade:
        df['End Time'].iloc[i]=df['Buy Time'].iloc[j]
        df['Trade Exit'].iloc[i]=df['Indicator'].iloc[j]
        break
  for i in range(0,len(df)):
    index_ltp,ce_options,pe_options=get_ce_pe_data(df['Symbol'].iloc[i],indexLtp=df['Close'].iloc[i])
    if df['Trade'].iloc[i]=="Buy":
      option_name=ce_options
    else:
      option_name=pe_options
    df['Options'].iloc[i]=option_name['symbol']
    df['token'].iloc[i]=option_name['token']
    df['qty'].iloc[i]=option_name['lotsize']
  for i in range(0,len(df)):
    if df['End Time'].iloc[i]=="-":
      df['End Time'].iloc[i]=datetime.datetime.now(tz=gettz('Asia/Kolkata')).replace(hour=15, minute=29, second=0, microsecond=0, tzinfo=None).time()
  for i in df.index:
    try:
  ...
  for i in opt_symbol_list:
    try:
      ...
      fromdate = from_date.strftime("%Y-%m-%d %H:%M")
      todate = to_date.strftime("%Y-%m-%d %H:%M")
      old_data=angel_data(i,"ONE_MINUTE","NFO",fromdate,todate)
      #old_data=get_historical_data(symbol="-",interval="1m",token=i,exch_seg="NFO",candle_type='NORMAL')
      #old_data=five_paisa_data(i,'1m','NFO',dat,dat)
  ...
  for j in range(0,len(df)):
    try:
      token=df['token'].iloc[j]
      tm=df['Buy Time'].iloc[j]
      end_tm=df['End Time'].iloc[j]
      old_data=optiols_ohlc_data[(optiols_ohlc_data['Token']==token)]
      old_data = old_data[(old_data['Datetime'] >= tm ) & (old_data['Datetime'] <= end_tm)]
      df['Buy'].values[j]=old_data['Open'].iloc[0]
      df['Close'].values[j]=old_data['Open'].iloc[-1]
      df['High'].values[j]=old_data['High'].max()
      df['Low'].values[j]=old_data['Low'].min()
    except Exception as e:
      print(e)
  return df
  ...
  for i in range(0,len(df)):
    try:
      ...
      if float(df['High'].iloc[i])>=float(df['Target'].iloc[i]):
        df['Profit'].iloc[i]=float(df['qty'].iloc[i])*(float(df['Target'].iloc[i])-float(df['Buy'].iloc[i]))
      else:
        df['Profit'].iloc[i]=float(df['qty'].iloc[i])*(float(df['Close'].iloc[i])-float(df['Buy'].iloc[i]))
      df['Win/Loss'].iloc[i]="Win" if float(df['Profit'].iloc[i])>= 0 else "Loss"
      df['ETE'].iloc[i]=float(df['qty'].iloc[i])*(float(df['Close'].iloc[i])-float(df['Buy'].iloc[i]))
      if float(df['High'].iloc[i])>=(float(df['Buy'].iloc[i])*1.3):
        df['30% Profit'].iloc[i]=int((1.3*float(df['Buy'].iloc[i])*float(df['qty'].iloc[i])))
      else:
        df['30% Profit'].iloc[i]=float(df['qty'].iloc[i])*(float(df['Close'].iloc[i])-float(df['Buy'].iloc[i]))
    except Exception as e:
      print(e)
  return df
