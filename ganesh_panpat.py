...
def trade_near_options(time_frame):
  ...
  nf_ltp=get_ltp_price("NIFTY")
  bnf_ltp=get_ltp_price("BANKNIFTY")
  sensex_ltp=get_ltp_price("SENSEX")
...
def update_position():
  try:
    ...
    if position==None:
      ...
      position['unrealised']=position['unrealised'].astype(float).round(2)
      realised=int(position['realised'].sum())
      unrealised=int(position['unrealised'].sum())
      position_updated.text(f"Position Ganesh: {now_time} Realised:{realised} UnRealised:{unrealised}")
  except Exception as e:
...
with tab0:
    ...
    with col1:
        ...
        algo_state=st.checkbox("Run Algo",value=True)
    with col2:
        log_holder=st.empty()
with tab1:
...
with tab4:
    ...
    with ind_col1:
        ...
        option_buy_indicator = st.multiselect('Option Indicator',indicator_list,['St Trade', 'ST_10_2 Trade','St Trade', 'ST_10_2 Trade'])
