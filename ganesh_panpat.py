...
def manual_buy(index_symbol,ce_pe="CE",index_ltp="-"):
  if index_ltp=="-":
    if symbol=="BANKNIFTY" or symbol=="^NSEBANK": index_ltp=st.session_state['Bank Nifty']
    if symbol=="NIFTY" or symbol=="^NSEI": index_ltp=st.session_state['=Nifty']
...
if algo_state:
  ...
  ss=1000*(60-datetime.datetime.now(tz=gettz('Asia/Kolkata')).second)
