...
def get_expiry_day_fut_token():
  ...
  st.session_state['expiry_day'] = expiry_df['expiry'].min()
  ...
  for i in range(0,len(df)):
    try:
      ...
      if time_frame=="1m" or time_frame=="ONE_MINUTE" or time_frame=="1min":
        ...
        if (ST=="Buy" or EMA=="Buy" or MA=="Buyy" or ST_10_2=="Buy" or HMA_Trade=="Buyy" or MA=='Buyy' or RSI_60=="Buy"):
          df['Trade'][i]="Buy"; df['Trade End'][i]="Buy"
        ...
        if (ST=="Buy" or MACD=="Buyy" or EMA=="Buy" or MA=="Buyy" or ST_10_2=="Buy" or RSI_60=="Buy"):
          df['Trade'][i]="Buy"; df['Trade End'][i]="Buy"
...
def get_todays_trade():
  try:
    ...
    buy_df[['price','LTP','Sell','quantity', 'Target','Stop Loss','Profit','Profit %']] = buy_df[['price','LTP','Sell','quantity', 'Target','Stop Loss','Profit','Profit %']].astype(int)
...
def supertrend_trail_order(open_position):
  if isinstance(open_position,str)==False:
    for i in open_position.index:
      try:
        symboltoken=open_position.loc[i]['symboltoken']
        tradingsymbol=open_position.loc[i]['tradingsymbol']
...
def sub_loop_code(now_time):
  ...
  #closing_index_trade()
  #ganesh_sl_trail()
  if now_time.minute==22: send_status()
  print_ltp()
...
if algo_trade:
  ...
  todays_trade_log=todays_trade_log[['updatetime','tradingsymbol','price','Stop Loss','Target','LTP','Status','Sell','Exit Time','Profit','Profit %','ordertag','Sell Indicator']]
