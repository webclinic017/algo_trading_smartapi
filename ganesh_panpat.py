...
def get_todays_trade(orderbook):
  try:
    ...
    sell_df['updatetime'] = pd.to_datetime(sell_df['updatetime']).dt.time
    buy_df['Sell']='-'
...
if algo_state:
  ...
  now_time=datetime.datetime.now(tz=gettz('Asia/Kolkata'))
  intradayclose = now_time.replace(hour=23, minute=56, second=0, microsecond=0)
  marketopen = now_time.replace(hour=9, minute=19, second=0, microsecond=0)
  marketclose = now_time.replace(hour=23, minute=56, second=0, microsecond=0)
  while True:
    try:
      now_time=datetime.datetime.now(tz=gettz('Asia/Kolkata'))
